...
file_handler = logging.FileHandler('seq2seq.log')
...
def gen_data():
    ...
    for i in range(len(data)-1):
        ...
        assign_pos = sorted(random.sample(range(len(next_line)), assign_cnt))
...
train_cnt = math.floor(len(all_X) * 0.8)
idx = [*range(len(all_X))]
random.shuffle(idx)
...
def main():
    ...
    encoder = Encoder(num_words, hidden_dim, n_layers=args.n_layers, bidirectional=args.bidirectional).to(device)
    if args.attn:
        decoder = AttnDecoder(hidden_dim, num_words, max_seqlen, n_layers=args.n_layers).to(device)
    else:
        decoder = Decoder(hidden_dim, num_words, n_layers=args.n_layers).to(device)
    ...
    if args.train:
        ...
        for e in range(start_epoch, start_epoch+epoch):
            ...
            logger.info("epoch {}, train_loss {:.4f}, valid_loss {:.4f}, best_epoch {}, best_loss {:.4f}, control_cnt {}, hit_cnt {}, percentage {:.4f}".format(
                         e, train_loss, valid_loss, best_epoch, best_loss, np.sum(all_control_cnt), np.sum(all_hit_cnt), percentage))
