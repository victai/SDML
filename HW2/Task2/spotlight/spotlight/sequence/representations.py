...
class MyLSTMNet(nn.Module):
    ...
    def __init__(self, num_items, embedding_dim=32,
        ...
        self.out_layer = nn.Linear(embedding_dim, num_items)
    ...
    def forward(self, user_representations, targets):
        ...
        predictions: tensor
            of shape (minibatch_size, sequence_length)
        """
        #out = self.out_layer(user_representations.sum(2).squeeze())
        out = self.out_layer(user_representations.max(2)[0])
...
class GRUNet(nn.Module):
    ...
    def user_representation(self, item_sequences):
        ...
        user_representations, _ = self.gru(sequence_embeddings)
        user_representations = nn.ReLU()(user_representations)
        user_representations = user_representations.permute(0, 2, 1)

        return user_representations[:, :, -2], user_representations[:, :, -1]
    ...
    def forward(self, user_representations, targets):
        ...
        #user_representations = nn.Linear(user_representations.size(-1), 32)(user_representations)
        #user_representations = nn.ReLU()(user_representations)
        user_representations = nn.Dropout(0.5)(user_representations)
        target_embedding = (self.item_embeddings(targets[:, -1]))
        target_embedding = nn.ReLU()(target_embedding)
        target_embedding = nn.Dropout(0.5)(target_embedding)
        #target_embedding = nn.Linear(target_embedding.size(-1), 32)(target_embedding)
        #target_embedding = nn.ReLU()(target_embedding)
        #print("target embedding", target_embedding.size())
        target_bias = self.item_biases(targets[:, -1]).squeeze()
        #print("target bias", target_bias.size())
        #print("user rep", user_representations.size())
