...
def my_sliding_window(tensor, timestamps, window_size, step_size=1):
    times, t_indices = np.unique(timestamps, return_index=True)

    for i in range(1, len(t_indices)):
        start_idx = t_indices[i]
        if i >= len(t_indices) - 1:
            stop_idx = len(tensor)
        else:
            stop_idx = t_indices[i+1]

        seq = tensor[max(start_idx - window_size + 1, 0): start_idx]  # seq = previous (window_size - 1) items + current day item.
        for idx in range(start_idx, stop_idx):
            yield np.hstack((seq, tensor[idx]))

def my_generate_sequences(user_ids, item_ids,
                          timestamps,
                          indices,
                          max_sequence_length):
    for i in range(len(indices)):
        ...
        for seq in my_sliding_window(item_ids[start_idx:stop_idx],
                                     timestamps[start_idx:stop_idx],
                                     max_sequence_length):
...
class Interactions(object):
    ...
    def to_sequence(self, max_sequence_length=10, min_sequence_length=None, step_size=None, mode="original"):
        """
        ...
        if mode == "mine":
            _, t_counts = np.unique(self.timestamps, return_counts=True)
            num_subsequences = int(t_counts[1:].sum())
        elif mode == "original":
        ...
        if mode == "mine":
            '''
            example:
                user U eats foods [0,5,8,999,7] at day D.
                sequence before day D is S (length = seq_len-1)
                generated sequences:
                    [S, 0]
                    [S, 5]
                    [S, 8]
            ...
            '''
            for i, (uid,
                    seq) in enumerate(my_generate_sequences(user_ids,
                                                            item_ids,
                                                            self.timestamps,
                                                            indices,
                                                            max_sequence_length)):
